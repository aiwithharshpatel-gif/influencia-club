...
const BrandMessages = () => {
  ...
  useEffect(() => {
    ...
    socket.on('message', (message) => {
      ...
      const isFromOrToActive = 
        (message.senderType === 'creator' && message.senderId === activeCreator) ||
        (message.recipientType === 'creator' && message.recipientId === activeCreator);
      ...
      setConversations(prev => {
        const partnerId = message.senderType === 'creator' ? message.senderId : message.recipientId;
        const existingIdx = prev.findIndex(c => c.creatorId === partnerId);
        if (existingIdx > -1) {
          const updated = [...prev];
          const conv = updated[existingIdx];
          const isCurrentActive = activeCreator === conv.creatorId;
          updated[existingIdx] = {
            ...conv,
            lastMessage: message.attachments?.length ? '📎 Attachment' : message.content,
            lastMessageAt: message.createdAt,
            unreadCount: isCurrentActive ? 0 : (message.recipientType === 'brand' ? conv.unreadCount + 1 : conv.unreadCount)
          };
          return updated.sort((a, b) => new Date(b.lastMessageAt) - new Date(a.lastMessageAt));
        } else {
          // Fetch conversations again to load creator details
          fetchConversations();
          return prev;
        }
      });
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const response = await api.post('/brand/messages/upload', formData, {
